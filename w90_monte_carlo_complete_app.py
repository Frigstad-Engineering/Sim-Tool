...
st.markdown(
    ...
    "[Input_for_simulations.xlsx](https://raw.githubusercontent.com/Frigstad-Engineering/Sim-Tool/main/Input_for_simulations.xlsx)"
)
...
if API_key:
    if st.button("⬇️  Download weather data via ERA5 API"):
        ...
        with st.spinner("Downloading project location weather (wind + waves) … this may take a few minutes"):
            try:
...
if df_project_weather is not None:
    if st.button("📊  Generate weather statistics & plots"):
        ...
        if "Wind_Direction" in dfw.columns:
            ...
            wr_df = dfw[["Adjusted_Windspeed","Wind_Direction"]].copy().apply(pd.to_numeric, errors="coerce").dropna()
            shifted_dir = (wr_df["Wind_Direction"] + 11.25) % 360
            dir_edges   = np.arange(0, 382.5, 22.5)
            wr_df["dir_bin"]   = pd.cut(shifted_dir, bins=dir_edges, labels=direction_labels, include_lowest=True, right=False)
            wr_df["speed_bin"] = pd.cut(wr_df["Adjusted_Windspeed"], bins=speed_bins, labels=speed_labels, include_lowest=True, right=False)
            ...
            for sc_lbl in speed_labels:
                ...
                ax_wr.bar(angles, vals, width=wr_width, bottom=wr_bottom, align="center", edgecolor="white", linewidth=0.8, label=sc_lbl)
                wr_bottom += vals
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("---")
st.header("🏗️  3–5.  Run All Simulations (Foundation + WTG)")
...
if not (uploaded_excel and df_project_weather is not None):
    ...
    if st.button("▶️  Run All Simulations", type="primary"):
        ...
        # ────────────────────────────────────────────────────────────────
        st.subheader("3.  Foundation Installation Simulations")
        mp_weather_results = {}
        prog_mp = st.progress(0, text="Running Foundation simulations …")
        vessel_list = list(_MP_CONFIGS.keys())

        for vi,(vessel,(sheet,cap)) in enumerate(_MP_CONFIGS.items()):
            with st.spinner(f"Foundation – {vessel} …"):
                try:
                    ...
                    mp_weather_results[vessel] = {"timeline":tl,"total":tot,"downtime":dwn,"summary":summ,"no_wx":tot}
                    p50 = round(summ["Total_Project_Days"].median(),1)
                    st.success(f"✅  Foundation {vessel} — P50 total: {p50} days")
                except Exception as e:
                    st.error(f"Foundation {vessel} failed: {e}")
        ...
        st.dataframe(pd.DataFrame(fdn_rows).set_index("Vessel"))

        # Full project summary (P0 = no-weather baseline)
        st.subheader("Foundation Full Project Summary (P0 = no-weather baseline)")
        fdn_full_frames = [build_project_summary(r["no_wx"],r["total"],r["downtime"],v)
                           for v,r in mp_weather_results.items()]
        st.dataframe(pd.concat(fdn_full_frames, ignore_index=True))

        # Bar chart
        st.subheader("Foundation Campaign Duration Bar Chart")
        fdn_bar_rows = []
        for v,r in mp_weather_results.items():
            s=r["summary"]
            tp=float(s["Total_Project_Days"].median()); dp=float(s["Total_Downtime_Days"].median())
            fdn_bar_rows.append({"Vessel":v,"Total_P50":round(tp,1),"Net_P50":round(tp-dp,1),
                                 "Downtime_Pct":round(dp/tp*100 if tp>0 else 0,1)})
        st.pyplot(plot_bar_comparison(fdn_bar_rows))

        # Foundation→WTG offset per vessel
        mp_end_offsets = {}
