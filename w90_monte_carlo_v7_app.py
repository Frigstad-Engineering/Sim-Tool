"""
W90 Monte Carlo – Streamlit App (v7)
...
Backwards-compatible:  all v6 simulation functions are kept and re-used.
"""
...
#  PAGE CONFIG
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(page_title="W90 Monte Carlo v7", page_icon="🌊", layout="wide")
st.title("🌊 W90 Monte Carlo – Offshore Wind Installation Simulator  ·  v7")
st.markdown("---")
...
with st.expander("⚙️  1. Project Inputs", expanded=True):
    ...
    st.caption(
        "For each selected vessel, choose one or more loading modes.  Each (vessel × mode) "
        "combination becomes a separate simulated case so they can be compared directly.\n\n"
        "• **Offshore**  — loaded at the project site, transit fixed at **0.5 hours**.\n"
        "• **Nearshore** — loaded at the barge location, transit distance = **barge distance**.\n"
        "• **Yard**      — loaded at the yard, transit distance = **dock distance**."
    )

    def _loading_mode_picker(vessel, enabled, default_modes):
        """Render a row of three checkboxes for one vessel; returns list of selected modes."""
        st.markdown(f"**{vessel}**" + ("" if enabled else "  *(vessel disabled above)*"))
        c1, c2, c3 = st.columns(3)
        with c1:
            m_off  = st.checkbox("Offshore",  value=("Offshore"  in default_modes),
                                 key=f"{vessel}_off",  disabled=not enabled)
        with c2:
            m_near = st.checkbox("Nearshore", value=("Nearshore" in default_modes),
                                 key=f"{vessel}_near", disabled=not enabled)
        with c3:
            m_yard = st.checkbox("Yard",      value=("Yard"      in default_modes),
                                 key=f"{vessel}_yard", disabled=not enabled)
        selected = []
        if enabled:
            if m_off:  selected.append("Offshore")
            if m_near: selected.append("Nearshore")
            if m_yard: selected.append("Yard")
        return selected

    W90_modes = _loading_mode_picker("W90", W90_sim, default_modes=["Yard", "Nearshore"])
    JUV_modes = _loading_mode_picker("JUV", JUV_sim, default_modes=["Yard"])
    FIV_modes = _loading_mode_picker("FIV", FIV_sim, default_modes=["Yard"])
    ...
    if st.button("⬇️  Download weather data via ERA5 API"):
        ...
        with st.spinner("Downloading project location weather (wind + waves) …"):
            try:
                ...
                st.success("✅  Project location weather downloaded.")
            except Exception as e:
                st.error(f"Project weather download failed: {e}")
        with st.spinner("Downloading barge location weather (wind only) …"):
            try:
                barge_bytes_api = _run_era5_download(
                    API_key, float(lat_barge), float(lon_barge),
                    ["10m_u_component_of_wind", "10m_v_component_of_wind"],
                    _BARGE_ZIP,
                )
                st.success("✅  Barge location weather downloaded.")
            except Exception as e:
        ...
        if proj_bytes_api and barge_bytes_api:
            ...
            st.success("✅  Both ERA5 downloads complete — weather data is ready.")
else:
...
# ── W90 WTG timeline (with parallel blocks) ──────────────────────────────────
def build_w90_timeline(excel_src, sheet_name, carry_cap, total_wtg, htd=1/24,
    ...
    lp  = raw.iloc[:lei + 1].reset_index(drop=True)
    ...
    def add(sq, ds, dr, p1, p2):
        rows.append({"Sequence": int(sq), "Description": ds,
                     ...
                     "Parallel_1": bool(p1), "Parallel_2": bool(p2),
                     "Inventory": int(inventory),
                     "WTG_Left": int(wtg_left), "Cycles_Left": int(cl())})

    while wtg_left > 0 or inventory > 0:
        if inventory == 0 and wtg_left > 0:
            for i in range(len(ld["seq"])):
                add(ld["seq"][i], ld["desc"][i], ld["dur"][i], ld["p1"][i], ld["p2"][i])
    ...
    while i < len(df):
        ...
        if not is_par:
            collapsed.append({"Sequence": int(df.at[i, "Sequence"]),
                              "Description": df.at[i, "Description"],
                              "Seq_Duration_Hours": float(df.at[i, "Seq_Duration_Hours"]),
                              "Inventory": int(df.at[i, "Inventory"]),
                              "WTG_Left": int(df.at[i, "WTG_Left"]),
                              "Cycles_Left": int(df.at[i, "Cycles_Left"]),
                              "Parallel_Block": False,
                              "Parallel_Branch_1_Hours": 0.0,
                              "Parallel_Branch_2_Hours": 0.0,
                              "Source_Sequences": str(int(df.at[i, "Sequence"]))})
            i += 1
        else:
            block = []; p1s = p2s = 0.0
            while i < len(df) and bool(df.at[i, "Parallel_1"] or df.at[i, "Parallel_2"]):
                ...
                i += 1
            last = block[-1]
            collapsed.append({"Sequence": int(block[0]["Sequence"]),
                              "Description": "Parallel block",
                              "Seq_Duration_Hours": max(p1s, p2s),
                              "Inventory": int(last["Inventory"]),
                              "WTG_Left": int(last["WTG_Left"]),
                              "Cycles_Left": int(last["Cycles_Left"]),
                              "Parallel_Block": True,
                              "Parallel_Branch_1_Hours": p1s,
                              "Parallel_Branch_2_Hours": p2s,
...
def add_duration_simulations_w90(timeline_df, excel_src, sheet_name, simulations=1, seed=42,
    ...
    """No-weather duration simulator for W90 WTG.  Single transit distance variant
    (per-mode runs are launched separately by the caller)."""
    raw = _excel_read(excel_src, sheet_name); raw.columns = [str(c).strip() for c in raw.columns]
    dur = raw[["N", "Seq Dur (hrs)", "Tr (hrs)", "Transit",
               "Parallell  1", "Parallell  2"]].copy()
    ...
    dur["Parallell  1"]  = dur["Parallell  1"].map(_norm_bool)
    dur["Parallell  2"]  = dur["Parallell  2"].map(_norm_bool)
    dur = dur.dropna(subset=["N"]).drop_duplicates(subset=["N"])
    dur["N"] = dur["N"].astype(int)
    tm  = dict(zip(dur["N"], dur["Transit"]))
    p1m = dict(zip(dur["N"], dur["Parallell  1"]))
    p2m = dict(zip(dur["N"], dur["Parallell  2"]))
    ...
    lm  = {s: (transit_distance / transit_speed if tm.get(s) else float(bl[s])) for s in bl}
    hm_ = {s: (transit_distance / transit_speed if tm.get(s) else float(bh[s])) for s in bh}
...
def simulate_weather_impacts_w90(timeline_df, project_weather_df, loading_weather_df,
                                  ...
                                  loading_mode="Yard"):
    """Weather sim for W90 WTG.  loading_mode controls which weather dataset the
    loading-phase rows are evaluated against:
       'Yard'      → loading_weather_df  (barge/dock-area wind only)
       'Nearshore' → loading_weather_df  (barge-area wind only)
       'Offshore'  → project_weather_df  (loaded at project site → wind + waves)
    Transit duration is determined by transit_distance / transit_speed."""
    transit_hours = float(transit_distance) / float(transit_speed)
    raw = _excel_read(excel_src, sheet_name); raw.columns = [str(c).strip() for c in raw.columns]
    ...
    op["Oplim Waves (m)"]  = pd.to_numeric(op["Oplim Waves (m)"], errors="coerce").fillna(np.inf)
    op["Oplim Wind (m/s)"] = pd.to_numeric(op["Oplim Wind (m/s)"], errors="coerce").fillna(np.inf)
    op["Parallell  1"] = op["Parallell  1"].map(_norm_bool)
    op["Parallell  2"] = op["Parallell  2"].map(_norm_bool)
    nt = ~op["Transit"]
    op.loc[nt, "Seq Dur (hrs)"] = op.loc[nt, "Seq Dur (hrs)"].fillna(op.loc[nt, "Tr (hrs)"])
    op.loc[nt, "Tr (hrs)"]      = op.loc[nt, "Tr (hrs)"].fillna(op.loc[nt, "Seq Dur (hrs)"])
    op.loc[op["Transit"], ["Seq Dur (hrs)", "Tr (hrs)"]] = transit_hours
    ...
    wvm = dict(zip(op["N"], op["Oplim Waves (m)"]))
    wdm = dict(zip(op["N"], op["Oplim Wind (m/s)"]))
    p1m = dict(zip(op["N"], op["Parallell  1"]))
    p2m = dict(zip(op["N"], op["Parallell  2"]))
    tm  = dict(zip(op["N"], op["Transit"]))
    base = timeline_df[["N", "Sequence", "Inventory", "WTG_Left", "Cycles_Left",
    ...
    for _, row in base.iterrows():
        ...
        rspecs.append({"is_parallel": is_par, "seqs": seqs,
                       ...
                       "wind_lim": float(min(wdm.get(s, np.inf) for s in seqs)),
                       "phase_type": pt})
    ...
    # For OFFSHORE loading mode, the loading is happening at the project site
    # so the loading-phase rows must also check waves.
    ...
    lt = wl["time"].to_numpy(dtype="datetime64[ns]")
    lw_waves = wl[wave_col].to_numpy(dtype=float) if loading_check_waves else None
    lnd = wl["Adjusted_Windspeed"].to_numpy(dtype=float)
...
def simulate_sequential_wtg_weather(timeline_df, weather_df, excel_src, sheet_name,
    ...
    w = w.dropna().sort_values("time").drop_duplicates(subset="time").reset_index(drop=True)
    wt  = w["time"].to_numpy(dtype="datetime64[ns]")
    ww_ = w[wave_col].to_numpy(dtype=float)
    wnd = w["Adjusted_Windspeed"].to_numpy(dtype=float)
    eli = eligible_indices if eligible_indices is not None else np.arange(len(wt))
    rng = np.random.default_rng(seed); n = len(base)
    tot_m = np.zeros((n, simulations), dtype=float)
...
def _enumerate_cases(vessel_info):
    """Return a list of (vessel, mode, label) tuples for every selected (vessel × mode)."""
    cases = []
    for vessel, info in vessel_info.items():
        if not info["selected"]:
            continue
        for mode in info["modes"]:
            cases.append((vessel, mode, f"{vessel} ({mode})"))
    return cases


if not (uploaded_excel and df_project_weather is not None and df_barge_wind is not None):
    st.info("Upload the operations Excel file and weather ZIPs above to enable simulations.")
else:
    vessel_info = _populate_vessel_info()
    cases = _enumerate_cases(vessel_info)
    ...
    if not cases:
        ...
        preview_rows = []
        for vessel, mode, label in cases:
    ...
    if can_run and st.button("▶️  Run All Simulations", type="primary"):
        ...
        if simulate_foundations:
            ...
            for ci, (vessel, mode, label) in enumerate(cases):
                info = vessel_info[vessel]
                ...
                prog_fd.progress((ci + 1) / len(cases))
            ...
            mp_end_offsets = {f"{v} ({m})": None for (v, m, _) in cases}
        ...
        case_labels = list(wtg_duration_results.keys())
        for vi, label in enumerate(case_labels):
            res    = wtg_duration_results[label]
                ...
                try:
                    if vessel == "W90":
                        tot, dwn, summ = simulate_weather_impacts_w90(
                            res["duration"], df_project_weather, df_barge_wind,
                            io.BytesIO(excel_bytes), sheet,
                            transit_distance=td, transit_speed=spd,
                            simulations=int(Simulations),
        ...
        if simulate_foundations and foundation_results:
            ...
            for lbl, r in wtg_weather_results.items():
                ...
                fdn = foundation_results.get(lbl)
                if fdn:
...
st.markdown("---")
st.caption(
    "W90 Monte Carlo v7 · Streamlit · "
    "Adds Jacket support and per-vessel loading-mode (Offshore / Nearshore / Yard) selection."
)
