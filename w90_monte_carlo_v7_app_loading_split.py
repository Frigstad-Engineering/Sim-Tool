    ...
    def _loading_mode_picker(vessel, enabled, default_modes, phase_key):
        ...
        with c1:
            m_off  = st.checkbox("Offshore",  value=("Offshore"  in default_modes),
                                 key=f"{phase_key}_{vessel}_off",  disabled=not enabled)
        with c2:
            m_near = st.checkbox("Nearshore", value=("Nearshore" in default_modes),
                                 key=f"{phase_key}_{vessel}_near", disabled=not enabled)
        with c3:
            m_yard = st.checkbox("Yard",      value=("Yard"      in default_modes),
                                 key=f"{phase_key}_{vessel}_yard", disabled=not enabled)
        selected = []
        if enabled:
