...
def coordinate_picker(label, default_lat, default_lon, key):
    ...
    if st.session_state[show_map_key]:
        if folium is None or st_folium is None:
            ...
            result = st_folium(m, height=350, width=None, key=f"{key}_map")
            clicked = result.get("last_clicked") if result else None

            if clicked:
                st.session_state[lat_state_key] = _round_to_era5_grid(clicked["lat"])
                st.session_state[lon_state_key] = _round_to_era5_grid(clicked["lng"])
                st.success(
                    f"{label} coordinates selected: "
                    f"{st.session_state[lat_state_key]:.2f}, "
                    f"{st.session_state[lon_state_key]:.2f} "
                    "(rounded to ERA5 0.25° grid)"
                )
                _safe_rerun()
